...
const renderError = (error, elements, translate) => {
  ...
  if (error !== 'Network Error') elements.feedback.textContent = translate(`errors.${error}`);
  elements.input.classList.add('is-invalid');
  elements.btn.disabled = false;
  elements.input.disabled = false;
};

const renderSuccess = (elements, state, translate) => {
  ...
  elements.feedback.textContent = translate('status.success');
...
const render = (state, elements, translate) => (path, value) => {
  const handlerByState = {
    ...
    error: () => renderError(state.form.error, elements, translate),
    success: () => renderSuccess(elements, state, translate),
  };

  switch (path) {
    case 'form.state':
      handlerByState[state.form.state]();
      break;
